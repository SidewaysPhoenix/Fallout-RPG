    ...
    function render() {
        ...
        data.forEach((rowData, rowIdx) => {
		  ...
		  if (storageKey === "fallout_weapon_table") {
		    ...
		    ammoWrap.style.alignItems = "center";
		    ammoWrap.style.gap = "8px";
		
		    const ammoLabel = document.createElement("span");
		    ammoLabel.textContent = "Ammo:";
		    ammoLabel.style.fontWeight = "bold";
		    ammoLabel.style.color = "#efdd6f";
		
		    const ammoInfo = findAmmoStacksForWeapon(rowData);
		
		    const ammoValueHost = document.createElement("span");
		
		    if (ammoInfo.mode === "none") {
			  ammoValueHost.textContent = "";
		    } else if (ammoInfo.mode === "infinite") {
			  ammoValueHost.textContent = "∞";
			  ammoValueHost.style.fontWeight = "bold";
			  ammoValueHost.style.color = "#efdd6f";
		    } else {
			  // If multiple ammo options, pick which option we’re using (UI framing)
			  // For now: default to first option; in the next pass we’ll persist selection per-weapon.
			  let chosenOption = ammoInfo.ammoOptions?.[0];
		
			  // Find first stack that matches chosen option (already qty-sorted least-first)
			  const firstStack = (ammoInfo.stacks && ammoInfo.stacks.length) ? ammoInfo.stacks[0] : null;
			  const startQty = firstStack ? firstStack.qty : 0;
		
			  const pm = createPlusMinusDisplay({
			    value: startQty,
			    min: 0,
			    max: 9999,
			    onChange: (val) => {
				  // If no stack exists yet, do nothing in this framing pass
				  // Next pass: optionally auto-create a gear row for missing ammo.
				  if (!firstStack) return;
				  setStackQty(firstStack.gearIndex, val);
			    }
			  });
		
			  ammoValueHost.appendChild(pm);
		
			  // Optional: show dropdown if weapon has multiple ammo options
			  if (ammoInfo.mode === "choose") {
			    const sel = document.createElement("select");
			    sel.style.background = "#fde4c9";
			    sel.style.color = "black";
			    sel.style.borderRadius = "6px";
			    sel.style.border = "1px solid #efdd6f";
			    sel.style.padding = "2px 6px";
		
			    ammoInfo.ammoOptions.forEach(opt => {
			    ...
			    sel.onchange = () => {
				  // Next pass: persist selected ammo option per weapon
				  chosenOption = sel.value;
				  if (typeof updateWeaponTableDOM === "function") updateWeaponTableDOM();
			    };
		
			    ammoWrap.appendChild(sel);
			  }
		    }
		
		    ammoWrap.append(ammoLabel, ammoValueHost);
		    ammoCell.appendChild(ammoWrap);
...
function setStackQty(gearIndex, newQty) {
  ...
  // ensure weapon UI reflects immediately
  if (typeof updateWeaponTableDOM === "function") updateWeaponTableDOM();
}
...
function parseAmmoOptions(ammoStr) {
  const raw = String(ammoStr ?? "").trim();
  if (!raw || raw.toLowerCase() === "n/a") return [];
  // Supports "A / B / C" (your note: "or ammo types separated by /")
