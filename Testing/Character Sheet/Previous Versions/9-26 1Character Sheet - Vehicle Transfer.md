...
function updateCarryWeightDisplay() {
  const strength = parseInt(document.getElementById("STR")?.value, 10) || 0;
  const bonus = Number(document.getElementById("CarryWeightBonus")?.value || 0) || 0;
  const base = 150 + (strength * 10);
  const max = base + bonus;
  const current = calculateCurrentCarryWeight();

  const baseEl = document.getElementById("BaseCarryWeightDisplay");
  ...
  if (baseEl) baseEl.textContent = formatWeightNumber(base);
  ...
  if (currentEl) {
    currentEl.style.color = overEncumbered ? "#ff8b80" : "#efdd6f";
  }
...
function renderStatsSection() {	
    ...
    infoGrid.style.gridTemplateColumns = "auto auto minmax(220px, 1fr)";
    infoGrid.style.gap = "5px 10px";
    ...
    function addRow(labelText, inputId, type="text", width="100%", spanWide=false) {
        ...
        input.style.color = "black";
        input.style.caretColor = 'black';
        input.style.gridColumn = spanWide ? "2 / 4" : "2";
    ...
    // Carry Weight occupies the unused lower-right area of Character Info.
    const carryColumn = document.createElement("div");
    carryColumn.style.gridColumn = "3";
    carryColumn.style.gridRow = "3 / 6";
    carryColumn.style.alignSelf = "stretch";
    carryColumn.style.display = "flex";
    carryColumn.style.flexDirection = "column";
    carryColumn.style.gap = "5px";
    carryColumn.style.minWidth = "220px";
    ...
    carryWrap.style.display = "grid";
    carryWrap.style.gridTemplateColumns = "1fr auto";
    carryWrap.style.gap = "5px 10px";
    carryWrap.style.transition = "border-color 0.15s, background-color 0.15s";

    function addCarryDisplay(labelText, id) {
      const label = document.createElement("span");
      label.textContent = labelText;
      label.style.color = "#FFC200";
      ...
      value.id = id;
      value.style.color = "#efdd6f";
      value.style.fontWeight = "bold";
      value.style.textAlign = "right";

      carryWrap.append(label, value);
    }

    addCarryDisplay("Base Carry Weight:", "BaseCarryWeightDisplay");

    const bonusLabel = document.createElement("label");
    bonusLabel.textContent = "Carry Weight Bonus:";
    bonusLabel.style.color = "#FFC200";

    const bonusInput = document.createElement("input");
    bonusInput.type = "number";
    bonusInput.id = "CarryWeightBonus";
    bonusInput.value = "0";
    bonusInput.style.width = "70px";
    bonusInput.style.justifySelf = "end";
    bonusInput.style.textAlign = "center";
    bonusInput.style.backgroundColor = "#fde4c9";
    bonusInput.style.color = "black";
    bonusInput.style.borderRadius = "5px";

    carryWrap.append(bonusLabel, bonusInput);
    addCarryDisplay("Max Carry Weight:", "MaxCarryWeightDisplay");
    addCarryDisplay("Current Weight:", "CurrentCarryWeightDisplay");
    ...
    infoGrid.appendChild(carryColumn);
...
function setupStatsSection() {
    ...
    const carryBonusInput = document.getElementById("CarryWeightBonus");
    if (carryBonusInput) {
      carryBonusInput.addEventListener("input", () => {
        saveInputs();
        updateCarryWeightDisplay();
      });
    }
...
async function fetchArmorData(section) {
    ...
    let armors = await Promise.all(armorFiles.map(async (file) => {
        let content = await app.vault.read(file);
		let stats = { link: file.basename, physdr: "0", raddr: "0", endr: "0", hp: "0", locations: "Unknown", value: "0", weight: "0" };
        let statblockMatch = content.match(/```statblock([\s\S]*?)```/);
        if (!statblockMatch) return stats;
        let statblockContent = statblockMatch[1].trim();
	  ...
	  openArmorItemPicker({
	    ...
	    onPick: (armor) => {
	      ...
	      const newData = {
	        ...
	        base: { physdr: armor.physdr, endr: armor.endr, raddr: armor.raddr, value: armor.value ?? "0", weight: armor.weight ?? "0" },
	        addons: []
	      };
    ...
    labels.forEach(([_, key]) => {
	  inputs[key].addEventListener('input', () => {
	    ...
	    if (deltaKey) {
	      ...
	      const modDelta = (stored.addons || []).reduce(
	        (sum, addon) => sum + (addon?.deltas?.[deltaKey] || 0),
	        0
	      );
...
function getChargeTrackedCoreType(itemOrName) {
  const raw = typeof itemOrName === "object"
    ? (itemOrName?.yamlName || itemOrName?.name || itemOrName?.link || "")
    : itemOrName;
...
function syncChargeTrackedCoreQty(rowData) {
  if (!isChargeTrackedCore(rowData)) return;
  if (!Array.isArray(rowData.chargeUnits)) rowData.chargeUnits = [];
  rowData.qty = String(rowData.chargeUnits.length);
}
...
function showCoreChargeEditor({ rowData, unit = null, onSave }) {
  ...
  cancelBtn.style = "background:#325886;color:#ffc200;font-weight:bold;padding:6px 18px;border-radius:6px;border:2px solid #ffc200;cursor:pointer;";
  ...
  saveBtn.onclick = () => {
    ...
    document.body.removeChild(overlay);
    onSave?.(isFusion ? { charges, maxCharges } : { charges });
  };
...
function renderChargeUnitsRow(rowData, visibleColumnCount, saveAndRender) {
  syncChargeTrackedCoreQty(rowData);
  const row = document.createElement("tr");
  row.classList.add("gear-charge-row");
...
function getItemIdentity(item) {
  return String(item?.sourcePath || item?.yamlName || stripWikiLink(item?.name || item?.link || "")).trim().toLowerCase();
}
...
function showVehicleTransferDialog({ rowData, onTransfer }) {
  ...
  if (isChargeTrackedCore(rowData)) {
    ...
    rowData.chargeUnits.forEach((unit, index) => {
      const label = document.createElement("label");
      label.style = "display:flex;align-items:center;gap:8px;background:#2e4663;padding:6px 8px;border-radius:6px;cursor:pointer;";
  ...
  cancel.style = "background:#325886;color:#ffc200;font-weight:bold;padding:6px 18px;border-radius:6px;border:2px solid #ffc200;cursor:pointer;";
...
async function backfillSavedWeights() {
  ...
  if (gearRows.length) {
    ...
    for (const row of gearRows) {
      if (String(row.weight ?? "").trim() !== "") continue;
