...
(() => {
  ...
  function coreType(item) {
    const clean = stripWikiLink(item?.yamlName || item?.name || item?.link || "").toLowerCase();
  ...
  function renderCargoPanel() {
    ...
    cargo.forEach((item, index) => {
      ...
      [stripWikiLink(item.name || item.link || ""), String(cargoQty(item)), String(item.cost ?? ""), `${String(item.weight ?? "")} (${formatWeightNumber(parseItemWeight(item.weight) * cargoQty(item))})`].forEach((value, ci) => {
        const td = document.createElement("td");
        td.textContent = value;
        td.style = `text-align:${ci===0?'left':'center'};padding:6px;color:#fde4c9;border-top:1px solid #294c75;`;
        tr.appendChild(td);
      });
