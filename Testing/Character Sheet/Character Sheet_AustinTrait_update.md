...
function renderStatsSection() {	
	...
	restBtn.onclick = () => {
	    ...
	    console.log(localStorage["fallout_perk_table"])
	};
...
rightCol.appendChild(luckWrapper);
	...
	// Compact Rad DMG display
	const radValueSpan = document.createElement("span");
	radValueSpan.textContent = radInitial;
	radValueSpan.style.minWidth = "18px";
	radValueSpan.style.textAlign = "center";
	radValueSpan.style.cursor = "pointer";
	radValueSpan.style.color = "#efdd6f";
	radValueSpan.style.fontWeight = "bold";
	radValueSpan.title = "Click to edit Radiation Damage";
	radValueSpan.addEventListener("mouseenter", () => {
	  radValueSpan.style.textDecoration = "underline";
	});
	radValueSpan.addEventListener("mouseleave", () => {
	  radValueSpan.style.textDecoration = "none";
	});

	
	// Plus/minus control (hidden by default)
	const radField = createPlusMinusDisplay({
	  value: radInitial,
	  min: 0,
	  onChange: (val) => {
	    radHiddenInput.value = val;
	    radHiddenInput.dispatchEvent(new Event("input", { bubbles: true }));
	    radValueSpan.textContent = val;
	  }
	});
	radField.dataset.pm = "RadDMG";
	radField.style.display = "none";
	
	// Toggle logic
	function showRadEditor() {
	  // Ensure the editor number always reflects the stored RadDMG
	  const n = radField.querySelector(".plusminus-num");
	  if (n) n.textContent = String(radHiddenInput.value ?? 0);
	
	  radValueSpan.style.display = "none";
	  radField.style.display = "flex";
	}
	
	function hideRadEditor() {
	  radField.style.display = "none";
	  radValueSpan.style.display = "inline-block";
	}
	
	// Click number → edit
	radValueSpan.addEventListener("click", (e) => {
	  e.stopPropagation();
	  showRadEditor();
	});
	...
	document.addEventListener("click", (e) => {
	  if (!radWrap.contains(e.target)) {
	    hideRadEditor();
	  }
	});
	
	// Escape / Enter → close
	radField.addEventListener("keydown", (e) => {
	...
	radWrap.appendChild(radValueSpan);
	radWrap.appendChild(radField);
	hpHeader.appendChild(hpTitle);
	hpHeader.appendChild(radWrap);
	hpWrapper.appendChild(hpHeader);
	...
	hpBarOuter.style.border = "2px solid #ffc200";
	hpBarOuter.style.borderRadius = "6px";
	...
	hpBarGreen.style.background = "#39ff14"; // terminal green
	...
	hpBarRed.style.background = "tomato";
	...
	const currentHpHiddenInput = document.createElement("input");
		...
		currentHpHiddenInput.addEventListener("input", () => {
		  // Sync the visible plus/minus number to the hidden value
		  const hpNum = currentHpField.querySelector(".plusminus-num");
		  if (hpNum) hpNum.textContent = String(currentHpHiddenInput.value ?? 0);
	...
	function clampCurrentHPToEffectiveMax() {
	  ...
	  if (cur > eff) {
	    ...
	    // keep the visible plusminus number in sync
	    const hpNum = currentHpField.querySelector(".plusminus-num");
	    if (hpNum) hpNum.textContent = String(eff);
	  }
	}
