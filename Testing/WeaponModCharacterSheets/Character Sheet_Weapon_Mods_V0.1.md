...
function createEditableTable({ columns, storageKey, fetchItems, cellOverrides = {} }) {
    ...
    function render() {
        ...
        data.forEach((rowData, rowIdx) => {
		  ...
		  if (storageKey === "fallout_weapon_table" && !Array.isArray(rowData.mods)) {
		    rowData.mods = [];
		  }
		  ...
		  if (storageKey === "fallout_weapon_table") {
		    ...
		    const mods = Array.isArray(rowData.mods) ? rowData.mods : [];
		    if (!mods.length) {
...
const WEAPON_MOD_FOLDERS = [
  "Fallout-RPG/Items/Mods/Weapon Mods", // adjust to your vault
];
...
function recalcWeaponCostFromMods(rowData) {
  ensureWeaponBaseCost(rowData);

  const baseNum = extractFirstInt(rowData.baseCost);
  if (Number.isNaN(baseNum)) {
    // If base cost isn't numeric, do not attempt to overwrite.
    return;
  }

  const mods = Array.isArray(rowData.mods) ? rowData.mods : [];
  const delta = mods.reduce((sum, m) => sum + (extractFirstInt(m.cost) || 0), 0);

  rowData.cost = String(baseNum + delta);
}

async function fetchWeaponModData() {
  if (cachedWeaponModData) return cachedWeaponModData;

  const allFiles = await app.vault.getFiles();
  const modFiles = allFiles.filter(f => WEAPON_MOD_FOLDERS.some(folder => f.path.startsWith(folder)));
  ...
  const mods = await Promise.all(modFiles.map(async (file) => {
    ...
    // Parse fields from your mod format (example: Ported Barrel.md)
    const get = (re) => {
      const m = block.match(re);
      return m ? m[1].trim().replace(/"/g, "") : "";
    };

    const modName = get(/mod:\s*(.+)/i) || file.basename;
    const cost = get(/cost:\s*(.+)/i); // e.g. "+35" :contentReference[oaicite:4]{index=4}

    return {
      id: file.path,                 // stable identifier
      basename: file.basename,
      link: `[[${file.basename}]]`,  // display as internal link
      cost: cost || "+0",
    };
  }));

  cachedWeaponModData = mods.filter(Boolean);
...
function openWeaponModPicker({ rowData, onAdded }) {
  ...
  const renderResults = async () => {
    ...
    const mods = await fetchWeaponModData();
    const filtered = !q
      ? mods
    ...
    filtered.forEach((m, idx) => {
      ...
      row.onmouseout = () => row.style.background = "inherit";

      row.onclick = () => {
        if (!Array.isArray(rowData.mods)) rowData.mods = [];

        // prevent duplicates by id (path)
        if (rowData.mods.some(x => x.id === m.id)) {
          if (typeof showSheetNotice === "function") showSheetNotice("Mod already added.");
          return;
        }

        rowData.mods.push({ id: m.id, link: m.link, cost: m.cost });
