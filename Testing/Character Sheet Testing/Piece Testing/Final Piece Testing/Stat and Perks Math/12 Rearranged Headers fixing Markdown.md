...
function addWeaponToTable(weapon, tbody, weapons) { 
    ...
    // 🔹 Name Column (Editable & Converts to Markdown Link)
    let nameTd = document.createElement('td'); 
    nameTd.style.border = '1px solid #ccc'; 
    nameTd.style.padding = '8px'; 

    function updateNameDisplay() {
        nameTd.innerHTML = weapon.link.replace(/\[\[(.*?)\]\]/g, '<a class="internal-link" href="$1">$1</a>');
    }
    
    updateNameDisplay(); // Initial render

    // Enable Editing on Click
    nameTd.addEventListener('click', () => {
        let input = document.createElement('input');
        input.type = 'text';
        input.value = weapon.link; // Show full markdown link while editing
        ...
        input.addEventListener('blur', () => {
            ...
            if (newValue) {
                ...
                weapon.link = newValue;
            }

            saveWeaponTableState(weapons);
            updateNameDisplay(); // Convert back to clickable markdown link
        });
