...
function updateWeaponStats() {
    console.log("🔄 updateWeaponStats() is running...");
    
    let weapons = JSON.parse(localStorage.getItem("fallout_weapon_table") || "[]"); // Load directly from storage
    console.log("📂 Loaded weapons before update:", weapons);

    weapons.forEach((weapon, index) => {
        let calculatedStats = calculateWeaponStats(weapon.type);

        // Find TN input & Tag checkbox
        let tnInput = document.querySelector(`#weapon-table-container tr:nth-child(${index + 2}) input[type="number"]`);
        let tagCheckbox = document.querySelector(`#weapon-table-container tr:nth-child(${index + 2}) input[type="checkbox"]`);

        if (tnInput) {
            tnInput.value = calculatedStats.TN;
            weapon.TN = calculatedStats.TN;  // ✅ Modify stored value
        }

        if (tagCheckbox) {
            tagCheckbox.checked = calculatedStats.Tag;
            weapon.Tag = calculatedStats.Tag;  // ✅ Modify stored value
        }
    });

    console.log("📂 Weapons after update (before saving):", weapons);

    // ✅ Force saving the updated list to storage
    localStorage.setItem("fallout_weapon_table", JSON.stringify(weapons));
    
    // 🔄 Re-render the weapon table with updated values
if (typeof renderWeaponTableUI === "function") {
    console.log("🔄 Re-rendering weapon table after stat update...");
    setTimeout(() => {
        document.getElementById("weapon-table-container").innerHTML = "";
        document.getElementById("weapon-table-container").appendChild(renderWeaponTableUI());
    }, 100);
} else {
    console.warn("⚠️ renderWeaponTableUI() is not defined.");
}


    console.log("💾 Weapons successfully saved to fallout_weapon_table!", JSON.parse(localStorage.getItem("fallout_weapon_table")));
}
...
createCharacterSheet(); 
    ...
    document.getElementById(stat)?.addEventListener("input", () => { 
        updateDerivedStats(); 
        saveInputs(); 
        updateWeaponStats(); // ✅ Ensures updates are reflected
    }); 
});

// Attach to skill inputs
Object.keys(skillToSpecial).forEach(skill => { 
    ...
    if (skillInput) { 
        skillInput.addEventListener("input", () => { 
            saveInputs();
            updateWeaponStats(); // ✅ Update weapon stats on skill change
        });
    } 
    if (skillTagInput) { 
