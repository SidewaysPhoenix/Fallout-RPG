...
const createCharacterSheet = () => {
    builder.createParagraph(
        `<div style="display:grid; grid-template-columns: 1fr; gap:20px; width:100%; min-width:550px; background-color:#325886; padding:15px; border-radius:8px;">
...
thead.appendChild(headerRow);
    ...
    }, 10); // Short delay to allow immediate return
...
function createEditableCell(value, key, weapon, weapons) {
    ...
    td.style.position = 'relative'; // Needed for detecting blank clicks

    function updateDisplay() {
        let text = weapon[key] || "";

        // Preserve all markdown links, but allow plain text too
        let formattedText = text.replace(/\[\[(.*?)\]\]/g, '<a class="internal-link" href="$1">$1</a>');

        td.innerHTML = formattedText;
        td.dataset.editing = "false"; // Allow clicking again
    }

    updateDisplay(); // Initial render

    // Enable Editing ONLY When Clicking Blank Space
    td.addEventListener('click', (event) => {
        if (td.dataset.editing === "true") return; // Prevent multiple activations

        // Check if the click was on a markdown link or the input field
        if (event.target.tagName === "A" || event.target.tagName === "INPUT") {
            return; // Do NOT start editing if clicking a link or active input
        }

        td.dataset.editing = "true"; // Mark as editing

        let input = document.createElement('input');
        input.type = 'text';
        input.value = weapon[key] || ""; // Show full text while editing
        ...
        function saveAndClose() {
            ...
            saveWeaponTableState(weapons);
            updateDisplay(); // Convert back to clickable markdown link after editing
            td.dataset.editing = "false"; // Ensure clicking inside does NOT reopen immediately
        }

        // ✅ Close edit mode when pressing Enter
        ...
        // ✅ Close edit mode when input loses focus (covers mobile back button)
    ...
    // 🔹 TN Column (Editable with Manual Override)
    let tnTd = document.createElement('td'); 
    let tnInput = document.createElement('input'); 
    tnInput.type = 'number'; 
    tnInput.value = weapon.TN ?? calculateWeaponStats(weapon.type).TN; 
    tnInput.style.width = '50px'; 

    if (weapon.manualTN !== undefined) {
        tnInput.value = weapon.manualTN;
    }

    tnInput.addEventListener('input', () => {
        if (tnInput.value.trim() !== "") {
            weapon.manualTN = tnInput.value; 
        } else {
            delete weapon.manualTN; 
        }
