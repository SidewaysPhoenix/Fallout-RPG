...
function addWeaponToTable(weapon, tbody, weapons) { 
    ...
    // 🔹 Add TN Column with Manual Override
    ...
    // If TN was manually entered before, restore that value
    if (weapon.manualTN !== undefined) {
    ...
    tnInput.addEventListener('input', () => {
        if (tnInput.value.trim() !== "") {
            weapon.manualTN = tnInput.value; // 🔹 Store manual TN entry
        } else {
            delete weapon.manualTN; // 🔹 Allow recalculating if cleared
        }
    ...
    removeTd.style.verticalAlign = 'middle'; 
