...
function addGearToTable(gear, tbody, gearList) {
    ...
    [gear.name, gear.description].forEach((_, index) => {
        ...
        function enableEditing() {
            let key = Object.keys(gear)[index];
            ...
            input.addEventListener('blur', () => {
                ...
                if (index === 1 && newValue.length > DESCRIPTION_LIMIT) {
                    newValue = newValue.substring(0, DESCRIPTION_LIMIT) + "...";
                }

                gearList[gearIndex][key] = newValue; // Update the correct entry in gearList
        ...
        let key = Object.keys(gear)[index];
        let displayText = document.createElement('span');
        displayText.innerHTML = gear[key].replace(/\[\[(.*?)\]\]/g, '<a class="internal-link" href="$1">$1</a>');
        displayText.style.cursor = 'pointer';
        displayText.onclick = enableEditing;
...
async function renderGearTableUI() {
    ...
    ["Name", "Description", "Remove"].forEach(headerText => {
