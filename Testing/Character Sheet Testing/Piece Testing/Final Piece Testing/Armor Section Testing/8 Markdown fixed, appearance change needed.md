...
function matchesSection(locations, section) {
    const mapping = {
        ...
        "Arms, Legs, Torso": ["Torso", "Left Arm", "Right Arm", "Left Leg", "Right Leg", "Outfit"] // ✅ Fully controlled mapping
    };
...
apparelInput.style.backgroundColor = '#fde4c9'; // Keep original field color
...
apparelDisplay.style.cursor = 'pointer';
apparelDisplay.style.padding = '5px';
apparelDisplay.style.border = '1px solid transparent';
apparelDisplay.style.backgroundColor = '#fde4c9'; 
...
function updateApparelDisplay() {
    if (storedData.apparel) {
        apparelDisplay.innerHTML = storedData.apparel.replace(/\[\[(.*?)\]\]/g, '<a class="internal-link" href="$1">$1</a>');
        apparelInput.value = storedData.apparel; // ✅ Ensure input has correct value
    } else {
...
apparelInput.addEventListener('blur', () => {
    storedData.apparel = apparelInput.value.trim(); // ✅ Keep input as raw text (No extra brackets)
    saveArmorData(section, storedData);
    updateApparelDisplay();

    // ✅ Switch to display mode only if text is present
    if (storedData.apparel) {
