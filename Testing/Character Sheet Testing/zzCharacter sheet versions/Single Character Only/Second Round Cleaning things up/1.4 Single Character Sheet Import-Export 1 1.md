...
function createEditableTable({ columns, storageKey, fetchItems, cellOverrides = {} }) {
    ...
    if (fetchItems) {
	    searchBar = createSearchBar({
	        fetchItems,
	        onSelect: (item) => {
    // Add weapon to storage as before
    let currentData = [];
    try { currentData = JSON.parse(localStorage.getItem(storageKey) || "[]"); } catch(e) {}
    currentData.push(item);
    localStorage.setItem(storageKey, JSON.stringify(currentData));

    // Recalculate stats
    if (storageKey === "fallout_weapon_table"
        && typeof updateWeaponStats === "function") {
        updateWeaponStats();
    }

    // Now, fully re-render the whole sheet UI (including new search bar and listeners)
