...
// --- Add to your UI, e.g. at the very top ---
function renderImportExportBar() {
    ...
    bar.style = "display:flex;gap:8px;align-items:center;background:#223657;padding:6px 10px 6px 10px;margin-bottom:18px;border-radius:7px;border:2px solid #ffc200";
...
function createSearchBar({ fetchItems, onSelect }) {
    ...
    input.addEventListener('input', debounce(async () => {
        ...
        matches.forEach((item, i) => {
            ...
            div.onclick = () => {
    // Defensive copy to avoid cache mutation, and only add if valid
...
const updateDerivedStats = () => { 
    ...
    if (inputs['Maximum HP'] && !inputs['Maximum HP']?.dataset?.manual) inputs['Maximum HP'].value = end + lck;
...
function renderStatsSection() {
    ...
    luckInput.style.caretColor = 'black';
    ...
    currHpInput.style.caretColor = 'black';
...
function setupStatsSection() {
    ...
    // Map all input fields by their id
    document.querySelectorAll("input").forEach(input => {
        const key = input.getAttribute("id");
        inputs[key] = input;
        input.addEventListener("input", saveInputs);
    ...
    // SPECIAL stat listeners for derived stats
    ["STR", "PER", "END", "CHA", "INT", "AGI", "LCK"].forEach(stat => {
        document.getElementById(stat)?.addEventListener("input", () => {
            updateDerivedStats();
            saveInputs();
    ...
    // Attach manual override listeners
    ["Maximum HP", "Initiative", "Defense", "MeleeDamage"].forEach(id => attachManualOverride(id));
...
.fallout-terminal-container {
    ...
    color: #38ff88;
    box-shadow: 0 0 40px 2px #18ff55b0;
    position: relative;
    overflow: hidden;
}
...
function refreshSheet() {
    ...
    setupStatsSection(); // <-- Attach listeners AFTER render
