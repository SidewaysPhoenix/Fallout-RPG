...
function createEditableTable({ columns, storageKey, fetchItems }) {
...
function createEditableCell({ rowData, col, onChange }) {
    ...
    // Editable cells for text/link/number
    let span = document.createElement('span');
    if (col.type === "link") {
        // Render Obsidian-style links
    ...
    // Handle click: only if not on a link!
    td.onclick = (event) => {
        if (event.target.tagName === "A" || event.target.tagName === "INPUT") return;
        // Already editing? Don't open a second editor.
...
function updateWeaponStats() {
    ...
    let tablecontainer = document.getElementById("weapon-table-container");
    if (!tablecontainer) {
        console.warn("⚠️ weapon-table-container not found. Creating it...");
        tablecontainer = document.createElement("div");
        tablecontainer.id = "weapon-table-container";
    ...
    setTimeout(() => {
        let newTable = renderWeaponTableSection();
        if (newTable instanceof Node) {
            tablecontainer.appendChild(newTable);
            console.log("✅ Weapon table successfully re-rendered!");
        } else {
            console.error("❌ renderWeaponTableSection() did not return a valid Node.");
        }
    }, 100);
}
...
function renderWeaponTableSection() {
    ...
    // Insert DRY search bar
container.appendChild(
    createSearchBar({
        fetchItems: fetchWeaponData,
        onSelect: (weapon) => {
            // Prevent duplicates
            if (!weapons.some(w => w.link === weapon.link)) {
                weapons.push(weapon);
    ...
    const headerRow = document.createElement('tr');
    [
        "Name", "TN", "Tag", "Damage", "Rate", "Effects", "Qualities", "Ammo",
        "Type", "Damage Type", "Range", "Weight", "Cost", "Remove"
    ].forEach(headerText => {
        const th = document.createElement('th');
    ...
    function createEditableCell(value, key, weapon, weapons) {
        ...
        td.addEventListener('click', (event) => {
            ...
            function saveAndClose() {
                ...
                if (newValue) weapon[key] = newValue;
                saveWeaponTableState(weapons);
                updateDisplay();
                td.dataset.editing = "false";
            }
			...
			input.addEventListener('keydown', (event) => {
			    if (event.key === "Enter" || event.key === "Escape") {
			        saveAndClose();
			        // Optionally, remove focus to ensure blur does not double-trigger
			        input.blur();
			    }
			});
    ...
    function addWeaponToTable(weapon, tbody, weapons) {
        ...
        // Other fields
        [
            "damage", "rate", "damage_effects", "qualities", "ammo",
            "type", "dmgtype", "range", "weight", "cost"
        ].forEach(key => {
    ...
    function loadWeaponTableState() {
        ...
        weapons.forEach(w => {
    if (w.TN === undefined || w.TN === null) {
        let newStats = calculateWeaponStats(w.type);
        w.TN = newStats.TN;
        w.Tag = newStats.Tag;
    }
});
    ...
    // Fetch weapon data (cached if possible)
...
sheetcontainer.appendChild(renderWeaponTableSection());
