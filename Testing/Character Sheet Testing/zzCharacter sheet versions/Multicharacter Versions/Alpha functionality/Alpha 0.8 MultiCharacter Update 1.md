...
falloutRPG.updateCharacterDropdown = function() {
    ...
    if (!dataList) {
        console.error("❌ ERROR: Character dropdown element not found.");
        return;
    }
    ...
    console.log("🔽 Updated dropdown with characters:", characters);
};
...
falloutRPG.getStorageKey = function(section) {
    let character = falloutRPG.sanitizeName(falloutRPG.getCurrentCharacter());
    return `fallout_rpg_${character}_${section}`;
};
...
// Switch to a new character and update all sections
// Ensure dropdown updates after character change
falloutRPG.switchCharacter = function(name) {
    console.log(`🔄 Switching character to: "${name}"`);

    if (!name || !name.trim()) {
        console.error("❌ ERROR: No character name provided to switchCharacter().");
        return;
    }

    let nameInput = document.getElementById("Name");
    if (!nameInput) {
        console.error("❌ ERROR: Name input field is missing from the DOM.");
        return;
    }

    localStorage.setItem("fallout_rpg_selected_character", name);
    nameInput.value = name; // 🔄 Update the input field

    document.dispatchEvent(new Event("characterChanged"));
};
...
falloutRPG.loadCharacterList = function() {
    return JSON.parse(localStorage.getItem("falloutRPG_Characters")) || [];
};
...
falloutRPG.saveCharacter = function(name) {
    console.log(`💾 Entered saveCharacter("${name}")`);
    ...
    let characters = falloutRPG.loadCharacterList();
    console.log("📋 Existing Characters:", characters);
    ...
    if (!characters.includes(name)) {
        ...
        console.log(`⚠ Character "${name}" already exists. Not saving.`);
    }
...
falloutRPG.deleteCharacter = function(name) {
    ...
    // 🔹 Remove character-specific data from storage
    ["info", "stats", "skills", "weapons", "armor"].forEach(section => {
        localStorage.removeItem(falloutRPG.getStorageKey(section));
    });

    if (characters.length > 0) {
        // 🔹 Switch to the first remaining character
        falloutRPG.switchCharacter(characters[0]);
    } else {
        console.log("🟠 No characters left. Resetting sheet...");
        localStorage.removeItem("fallout_rpg_selected_character");

        // 🔄 Reset the dropdown to "New Character..."
        falloutRPG.updateCharacterDropdown();
...
const createCharacterSheet = () => {
   ...
   <div style="display: flex; align-items: center; gap: 5px; position: relative;">
    <input id="Name" type="text" list="character-list" placeholder="Enter or Select Name" 
        style="width: 100%; background-color:#fde4c9; border-radius:5px; color:black;">
