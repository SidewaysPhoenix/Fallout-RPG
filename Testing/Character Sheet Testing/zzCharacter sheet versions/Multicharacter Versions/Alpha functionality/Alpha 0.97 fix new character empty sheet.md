...
falloutRPG.switchCharacter = function(name) {
    ...
    document.getElementById("character-select").value = name; // 🔄 Update input field

    // ✅ Set the dropdown selection to the current name
...
falloutRPG.saveNewCharacter = function () {
    ...
    console.log(`🗑️ Cleared any previous stored data for new character: "${newName}"`);
    ...
    // ✅ Reload character list **with a short delay**
    setTimeout(() => {
        falloutRPG.updateCharacterDropdown();
        characterSelect.value = newName;  // ✅ Force selection of the new character
        console.log(`✅ Character "${newName}" saved and dropdown updated.`);
    }, 50);
