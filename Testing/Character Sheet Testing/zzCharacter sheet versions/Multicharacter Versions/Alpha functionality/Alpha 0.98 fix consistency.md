...
falloutRPG.updateCharacterDropdown = function() {
    setTimeout(() => {
        ...
        let selectedBeforeUpdate = select.value;
        
        select.innerHTML = ""; // ✅ Always clear previous options

        if (!currentCharacter && !selectedBeforeUpdate) {
	    ...
	    else if (selectedBeforeUpdate && characters.includes(selectedBeforeUpdate)) {
...
falloutRPG.switchCharacter = function(name) {
    console.log(`🔄 Switching character to: "${name}"`);
    ...
    if (characterSelect) {
        ...
        console.log(`✅ Dropdown updated to selected character: "${name}"`);
    } else {
        console.error("❌ ERROR: Character dropdown not found.");
    }
    ...
    setTimeout(() => {
        ...
        console.log(`📜 Loaded data for character: "${name}"`);
    }, 100);
};
...
falloutRPG.deleteCharacter = function(name) {
    let characters = falloutRPG.loadCharacterList();
    if (!characters.includes(name)) {
        console.warn(`⚠ Character "${name}" not found in list.`);
        return;
    }

    if (!confirm(`Delete "${name}"? This cannot be undone.`)) return;

    // ✅ Fix Typo: Correctly filter out the deleted character
    characters = characters.filter(c => c !== name);
    localStorage.setItem("falloutRPG_Characters", JSON.stringify(characters));

    let key = `fallout_rpg_${falloutRPG.sanitizeName(name)}`;
    localStorage.removeItem(key);  // ✅ Remove character data from storage

    // ✅ If the deleted character is currently selected, reset the selection
    let currentCharacter = localStorage.getItem("fallout_rpg_selected_character");
    ...
    // ✅ If there are remaining characters, switch to the first one
    if (characters.length > 0) {
        falloutRPG.switchCharacter(characters[0]);
    }
...
falloutRPG.clearCharacterSheet = function () {
    console.log("🧹 Resetting all character fields...");

    // ✅ Find all input fields and set them to empty
    let inputs = document.querySelectorAll(".character-field input, .character-field textarea");
    inputs.forEach(input => {
        input.value = "";
    });

    // ✅ Uncheck all checkboxes
    let checkboxes = document.querySelectorAll(".character-field input[type='checkbox']");
    checkboxes.forEach(checkbox => {
        checkbox.checked = false;
    });

    // ✅ Reset dropdown selections
    let dropdowns = document.querySelectorAll(".character-field select");
    dropdowns.forEach(dropdown => {
        dropdown.selectedIndex = 0;
    });

    // ✅ REMOVE THE STORED DATA FOR CURRENT CHARACTER
    let currentCharacter = localStorage.getItem("fallout_rpg_selected_character");
    if (currentCharacter) {
        let storageKey = `fallout_rpg_${falloutRPG.sanitizeName(currentCharacter)}`;
        localStorage.removeItem(storageKey);
        console.log(`🗑️ Cleared storage for character: "${currentCharacter}"`);
    } else {
        console.warn("⚠ No selected character to clear.");
    }

    console.log("✅ Character sheet fully cleared.");
};
...
setTimeout(() => {
    ...
    console.log("✅ Name input updated to force full list on click and typing.");
}, 500);
...
setTimeout(() => {
    let characterSelect = document.getElementById("character-select");
    if (characterSelect) {
        falloutRPG.updateCharacterDropdown(); // ✅ Load the dropdown
        falloutRPG.loadCharacterData();      // ✅ Load the last selected character's data
        console.log("✅ Character data loaded on page load.");
    } else {
...
console.log("✅ Attaching event to Add Character button...");
...
falloutRPG.handleAddCharacter = function () {
    console.log("🟢 FINAL TEST: Add Character button clicked!");
    ...
    console.log(`💾 Attempting to save character: "${name}"`);
...
falloutRPG.enterCharacterCreationMode = function () {
    console.log("🆕 Entering character creation mode...");
...
falloutRPG.saveNewCharacter = function () {
    ...
    // ✅ ENSURE THE STORAGE KEY IS UNIQUE FOR THIS NEW CHARACTER
    let newStorageKey = `fallout_rpg_${falloutRPG.sanitizeName(newName)}`;
    if (localStorage.getItem(newStorageKey)) {
        console.warn(`⚠ WARNING: Character "${newName}" already exists.`);
    } else {
        console.log(`🆕 Creating new character entry: "${newName}"`);
    }

    // ✅ UPDATE SELECTED CHARACTER IN STORAGE
    localStorage.setItem("fallout_rpg_selected_character", newName);
    console.log(`🔄 Set selected character to: "${newName}"`);

    // ✅ ENSURE NEW CHARACTER STARTS BLANK
    falloutRPG.clearCharacterSheet();
    console.log("🧹 Ensuring new character starts with an empty sheet.");
...
setTimeout(() => {
    ...
    console.log("🔄 Reattached event listener to dropdown.");
}, 50);
    ...
    // ✅ DELAY DROPDOWN UPDATE TO PREVENT UI ISSUES
    setTimeout(() => {
        falloutRPG.updateCharacterDropdown();
        console.log("🔄 Character dropdown updated.");
    }, 40);

    // ✅ DELAY CHARACTER SWITCHING TO ENSURE UI IS READY
    setTimeout(() => {
...
setTimeout(() => {
    ...
    if (selectedCharacter) {
        falloutRPG.switchCharacter(selectedCharacter);
        console.log(`✅ Loaded character: ${selectedCharacter}`);
    }
});


    console.log("✅ Character select dropdown is now active.");
}, 500);
