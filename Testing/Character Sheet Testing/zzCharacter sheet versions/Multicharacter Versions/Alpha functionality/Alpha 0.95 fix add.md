...
falloutRPG.updateCharacterDropdown = function() {
    setTimeout(() => {
        ...
        if (!select) {
            console.error("❌ ERROR: Character dropdown not found. Retrying...");
            setTimeout(falloutRPG.updateCharacterDropdown, 500); // Try again after 500ms
            return;
        }
        ...
        if (characters.length === 0) {
            console.warn("⚠ No characters found. Dropdown is empty.");
        } else {
    ...
    if (currentCharacter && characters.includes(currentCharacter)) {
        ...
        console.log(`🔄 Restoring last selected character: "${currentCharacter}"`);
    } else {
        console.warn("⚠ No previously selected character found in storage.");
    }

        console.log("✅ Updated character dropdown with full list:", characters);
    }, 500);
};
