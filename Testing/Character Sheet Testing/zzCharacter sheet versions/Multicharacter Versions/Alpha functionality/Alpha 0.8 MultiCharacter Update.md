```js-engine
window.falloutRPG = window.falloutRPG || {}; // Ensure global namespace
...
// Generate storage key per section
...
// Update all sections when switching characters
...
falloutRPG.characters = JSON.parse(localStorage.getItem("fallout_rpg_character_list")) || [];

falloutRPG.saveCharacter = function(name) {
    if (!name.trim()) return;
    if (falloutRPG.characters.includes(name)) {
        alert("Character name already exists!");
        return;
    }
    falloutRPG.characters.push(name);
    localStorage.setItem("fallout_rpg_character_list", JSON.stringify(falloutRPG.characters));
    falloutRPG.switchCharacter(name);
...
falloutRPG.deleteCharacter = function(name) {
    ...
    falloutRPG.characters = falloutRPG.characters.filter(char => char !== name);
    localStorage.setItem("fallout_rpg_character_list", JSON.stringify(falloutRPG.characters));
    
    // Remove all stored data for the character
    let sections = ["weapons", "armor", "perks", "gear", "caps"];
    sections.forEach(section => localStorage.removeItem(falloutRPG.getStorageKey(section)));

    falloutRPG.updateCharacterDropdown();
    
    if (falloutRPG.characters.length > 0) {
        falloutRPG.switchCharacter(falloutRPG.characters[0]);
    } else {
        document.dispatchEvent(new Event("resetSheet"));
    }
};
...
falloutRPG.updateCharacterDropdown = function() {
    ...
    dropdown.innerHTML = "";
    falloutRPG.characters.forEach(char => {
...
// Lock/Unlock fields
falloutRPG.toggleFields = function(state) {
    document.querySelectorAll(".character-field").forEach(field => {
        field.disabled = !state;
    });
};

document.addEventListener("resetSheet", () => falloutRPG.toggleFields(false));
document.addEventListener("characterChanged", () => falloutRPG.toggleFields(true));

// Initialize on page load
document.addEventListener("DOMContentLoaded", () => {
    falloutRPG.updateCharacterDropdown();
    let storedCharacter = falloutRPG.getCurrentCharacter();
    if (storedCharacter) {
        falloutRPG.switchCharacter(storedCharacter);
    } else {
        document.dispatchEvent(new Event("resetSheet"));
    }
});
...
const createCharacterSheet = () => {
    ...
    <input id="Name" type="text" style="background-color:#fde4c9; border-radius:5px; color:black; margin-Bottom: 5px">
    <!-- Add Button (➕) -->
