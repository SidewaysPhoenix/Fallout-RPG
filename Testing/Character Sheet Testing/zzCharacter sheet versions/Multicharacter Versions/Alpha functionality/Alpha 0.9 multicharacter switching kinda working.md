...
falloutRPG.updateCharacterDropdown = function() {
    ...
    // Default "Select a Character" option
    ...
    // Populate dropdown with saved characters
...
falloutRPG.switchCharacter = function(name) {
    ...
    document.getElementById("Name").value = name; // 🔄 Update input field
...
setTimeout(() => {
    falloutRPG.updateCharacterDropdown();
    console.log("✅ Forced character dropdown update on page load.");
}, 100);

setTimeout(() => {
    let nameInput = document.getElementById("Name");
    let dataList = document.getElementById("character-list");

    if (!nameInput || !dataList) {
        console.error("❌ ERROR: Name input or character-list datalist not found.");
        return;
    }
...
setTimeout(() => {
    ...
    addBtn.removeEventListener("click", falloutRPG.handleAddCharacter); // Ensure no duplicates
    addBtn.addEventListener("click", falloutRPG.handleAddCharacter);
