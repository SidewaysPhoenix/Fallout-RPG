...
falloutRPG.switchCharacter = function(name) {
    ...
    // 🔄 Reload entire character data
    falloutRPG.loadCharacterData();
    setTimeout(() => document.dispatchEvent(new Event("characterChanged")), 200);
};
...
falloutRPG.deleteCharacter = function(name) {
    ...
    characters = charachcters.filter(c => c !== name);
    localStorage.setItem("falloutRPG_Characters", JSON.stringify(characters));

    let key = `fallout_rpg_${falloutRPG.sanitizeName(name)}`;
    localStorage.removeItem(key);  // 🔹 Remove all character data
    ...
    console.log(`🗑️ Character "${name}" deleted.`);
};
