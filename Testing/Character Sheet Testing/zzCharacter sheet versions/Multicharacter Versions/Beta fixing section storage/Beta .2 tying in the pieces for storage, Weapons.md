...
function updateCapsUI() {
    ...
    console.log(`🔄 Caps UI updated for character: "${localStorage.getItem("fallout_rpg_selected_character")}"`);
}
...
falloutRPG.deleteCharacter = function () {
    ...
    // ✅ Delay Dropdown Update to Prevent UI Lag
    requestAnimationFrame(() => {
        falloutRPG.updateCharacterDropdown();
    });

    // ✅ Ensure Character Sheet Clears Immediately
    requestAnimationFrame(() => {
        falloutRPG.clearCharacterSheet();
    });

    // ✅ Select "Select a Character" Instead of Empty Option
    setTimeout(() => {
        select.value = "";
    }, 50);
};
...
falloutRPG.saveNewCharacter = function () {
	...
	localStorage.removeItem(getWeaponStorageKey()); // ✅ Clear weapons for new character
...
let storedValue = loadCaps();
    ...
    console.log(`📩 Received event: characterSwitched -> "${event.detail.character}"`);
    updateCapsUI();
});
...
function updateCapsUI() {
    ...
    console.log(`🔄 Caps UI updated for "${localStorage.getItem("fallout_rpg_selected_character")}"`);
}
...
document.addEventListener("characterSwitched", () => {
    console.log("📢 Character switched, updating weapon table...");
    refreshWeaponTable();
});
