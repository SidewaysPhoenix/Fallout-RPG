...
// Ensure falloutRPG exists
window.falloutRPG = window.falloutRPG || {};

falloutRPG.updateCharacterDropdown = function() {
    setTimeout(() => {
        let select = document.getElementById("character-select");
        let characters = JSON.parse(localStorage.getItem("fallout_rpg_character_list")) || [];
        let currentCharacter = localStorage.getItem("fallout_rpg_selected_character");

        select.innerHTML = ""; // ✅ Always clear previous options
        ...
        // ✅ Only set the dropdown if the character still exists
        if (currentCharacter && characters.includes(currentCharacter)) {
            select.value = currentCharacter;
        } else {
            select.value = "";
            localStorage.removeItem("fallout_rpg_selected_character"); // ✅ Ensure storage is cleaned
        }
    }, 50);
};
...
falloutRPG.getCurrentCharacter = function() {
    return localStorage.getItem("fallout_rpg_selected_character") || "";
};

// Generate a unique storage key for any section
// Sanitize character names to prevent storage key issues
falloutRPG.sanitizeName = function(name) {
    return name.replace(/[^a-zA-Z0-9_]/g, "_"); // Removes special characters
};

// Modify getStorageKey to use sanitized names
falloutRPG.getStorageKey = function() {
    let character = falloutRPG.sanitizeName(falloutRPG.getCurrentCharacter());
    return `fallout_rpg_${character}`;
};
...
falloutRPG.switchCharacter = function(name) {
 
function updateCapsUI() {
    if (!CapsDisplay) return;
    CapsDisplay.textContent = loadCaps();
    CapsInput.value = loadCaps();
}



    // ✅ Save the selected character to storage
    localStorage.setItem("fallout_rpg_selected_character", name);
    ...
    // ✅ Ensure the dropdown list reflects the latest changes
    falloutRPG.updateCharacterDropdown();

    // ✅ Load the saved character's data
    setTimeout(() => {
        falloutRPG.loadCharacterData();
        document.dispatchEvent(new CustomEvent("characterSwitched", {
...
falloutRPG.loadCharacterList = function() {
    let stored = localStorage.getItem("falloutRPG_Characters");
...
falloutRPG.saveCharacter = function(name) {
    if (!name || !name.trim()) {
        alert("Enter a character name!");
        return;
    }

    let characters = falloutRPG.loadCharacterList();
    if (!characters.includes(name)) {
        characters.push(name);
        localStorage.setItem("falloutRPG_Characters", JSON.stringify(characters));
    }

    // Save character-specific data
    let key = `fallout_rpg_${name}`;
    let data = {
        info: {}, stats: {}, skills: {}, weapons: {}, armor: {}, ammo: {}, gear: {}
    };
...
falloutRPG.deleteCharacter = function () {
    let select = document.getElementById("character-select");
    let characterToDelete = select.value;
    ...
    let confirmDelete = confirm(`Are you sure you want to delete "${characterToDelete}"?`);
    if (!confirmDelete) return;


    // ✅ Remove from localStorage
    let storageKey = `fallout_rpg_${falloutRPG.sanitizeName(characterToDelete)}`;
    localStorage.removeItem(storageKey);

    // ✅ Update stored character list
    let characters = JSON.parse(localStorage.getItem("fallout_rpg_character_list")) || [];
    let updatedCharacters = characters.filter(name => name !== characterToDelete);
    localStorage.setItem("fallout_rpg_character_list", JSON.stringify(updatedCharacters));

	requestAnimationFrame(() => {
        falloutRPG.clearCharacterSheet();
        falloutRPG.updateCharacterDropdown();
        select.value = ""; // Reset dropdown selection
        localStorage.removeItem("fallout_rpg_selected_character");
        console.log("✅ Character successfully deleted and UI updated.");
    });
};


falloutRPG.clearCharacterSheet = function () {
...
setTimeout(() => {
    let nameInput = document.getElementById("character-select");
    if (!nameInput) {
        console.error("❌ ERROR: Character select dropdown not found.");
        return;
    }

    // Ensure clicking the name input always shows the full list
    nameInput.addEventListener("click", function () {
        this.value = ""; // ✅ Clears the field to show all names 
    falloutRPG.updateCharacterDropdown();
    });

    // Prevent filtering when typing (disables native datalist filtering)
...
setTimeout(() => {
    ...
    if (characterSelect) {
        ...
        console.error("❌ ERROR: character-select not found at page load. Retrying...");
...
falloutRPG.saveCharacterData = function() {
    let character = falloutRPG.getCurrentCharacter();
    if (!character) return;

    let key = falloutRPG.getStorageKey();
    let data = JSON.parse(localStorage.getItem(key) || '{}');

    // Save all inputs (Character Info)
    Object.keys(inputs).forEach(id => {
        data.info = data.info || {};
        data.info[id] = inputs[id].value || "";
    });
...
falloutRPG.loadCharacterData = function() {
    let character = falloutRPG.getCurrentCharacter();
    if (!character) return;
    ...
    Object.keys(inputs).forEach(id => {
        if (data.info && data.info[id] !== undefined) {
            inputs[id].value = data.info[id];
        }
    });
};
        ...
        `<div style="display:grid; grid-template-columns: 1fr; gap:20px;  min-width:700px; background-color:#325886; padding:15px; border-radius:8px;">
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;  align-items:start; ">
                ...
                <div style="border:1px solid #ccc; padding:10px; border-radius:8px;">
                    ...
                    <div style="display: grid; grid-template-columns: auto 1fr auto auto; align-items: center; gap: 5px; width: 100%;">
    <label style="color:#FFC200;margin-Bottom: 5px">Name:</label>
   <div style="align-items: center; gap: 5px;">
    <select id="character-select" style="width: 100%; border-radius:5px;margin-Bottom: 5px">
    <option value="">Select a Character</option>
</select>
</div>
    <!-- Add Button (➕) -->
    <button id="add-character" style="background:#23a745; color:white; border:none; padding:3px 6px; border-radius:5px; font-size:14px; margin-Bottom: 5px">➕</button>
    <!-- Delete Button (🗑️) -->
    <button id="delete-character" style="background:#dc3545; color:white; border:none; padding:3px 6px; border-radius:5px; font-size:14px; margin-Bottom: 5px">🗑️</button>
</div>                         
                        <div style="display: grid; grid-template-columns: auto 1fr; gap: 5px; align-items: center; width: 100%;">
    <label style="color:#FFC200;margin-Bottom: 5px">Origin:</label>
    <input id="Origin" type="text" style="width: 100%; background-color:#fde4c9; border-radius:5px; color:black; margin-Bottom: 5px;">
						</div>    
    <div style="display: grid; grid-template-columns: auto auto auto; gap: 5px; align-items: center;">
    <label style="color:#FFC200;margin-Bottom: 5px">Level:</label>
    <input id="Level" type="number" style="width:75px; background-color:#fde4c9; border-radius:5px; color:black;margin-Bottom: 5px; margin-Left: 5px;">						
						<div style="border:1px solid #efdd6f; padding:10px; display:grid; grid-template-columns: auto 1; align-items:center;">
    <label style="color:#FFC200">XP Earned:</label>
    <input id="XPEarned" type="number" style="width: 100%; background-color:#fde4c9; border-radius:5px; color:black;">
    <label style="color:#FFC200">XP to Next Level:</label>
    <input id="XPNext" type="number" style="width:100%; background-color:#fde4c9; border-radius:5px; color:black;">
						</div>
    </div>
</div>          
...
setTimeout(() => {
    ...
    if (!addBtn) {
        console.error("❌ ERROR: Add Character button not found.");
        return;
    }
    addBtn.removeEventListener("click", falloutRPG.enterCharacterCreationMode);
    addBtn.addEventListener("click", falloutRPG.enterCharacterCreationMode);
}, 500);



setTimeout(() => {
    let deleteBtn = document.getElementById("delete-character");
    if (!deleteBtn) {
    ...
    deleteBtn.removeEventListener("click", falloutRPG.deleteCharacter);
    deleteBtn.addEventListener("click", function() {
        let selectedCharacter = falloutRPG.getCurrentCharacter();
        if (!selectedCharacter) {
            alert("No character selected to delete.");
            console.warn("⚠ No character selected when delete was attempted.");
            return;
        }
        falloutRPG.deleteCharacter(selectedCharacter);
    });
}, 500);
...
// Define function outside to prevent issues
falloutRPG.handleAddCharacter = function () {
    let nameInput = document.getElementById("character-select");
    if (!nameInput) {
    ...
    if (!name) {
        alert("Enter a valid character name.");
        console.error("❌ No name entered.");
        return;
    }
    if (typeof falloutRPG.saveCharacter === "function") {
        falloutRPG.saveCharacter(name);
    } else {
        console.error("❌ ERROR: falloutRPG.saveCharacter is not defined.");
    }
};

falloutRPG.enterCharacterCreationMode = function () {
    let characterSelect = document.getElementById("character-select");
    let addBtn = document.getElementById("add-character");

    if (!characterSelect || !addBtn) {
        console.error("❌ ERROR: Character select dropdown or Add button not found.");
        return;
    }
    ...
    inputField.id = "new-character-input";
    inputField.placeholder = "Enter new character name";
    inputField.style.width = "100%";
    inputField.style.backgroundColor = "#fde4c9";
    ...
    characterSelect.replaceWith(inputField);
	inputField.focus();
    // Change Add Button into a Save Button
    addBtn.textContent = "✔ Save";
    addBtn.removeEventListener("click", falloutRPG.enterCharacterCreationMode);
    addBtn.addEventListener("click", falloutRPG.saveNewCharacter);
};

falloutRPG.saveNewCharacter = function () {
    let inputField = document.getElementById("new-character-input");
    let addBtn = document.getElementById("add-character");
    if (!inputField || !addBtn) {
        console.error("❌ ERROR: New character input field or Add button not found.");
        return;
    }
    ...
    console.log(`📋 Attempting to save character: "${newName}"`);
   
	// ✅ Retrieve stored character list and update it
    let characters = JSON.parse(localStorage.getItem("fallout_rpg_character_list")) || [];
    if (characters.includes(newName)) {
        alert(`⚠ Character "${newName}" already exists.`);
        return;
    }
    characters.push(newName);
    localStorage.setItem("fallout_rpg_character_list", JSON.stringify(characters));
    console.log(`✅ Updated character list:`, characters);

    // ✅ Ensure unique storage key for this character
    let newStorageKey = `fallout_rpg_${falloutRPG.sanitizeName(newName)}`;
    localStorage.setItem("fallout_rpg_selected_character", newName);

    // ✅ Start character with a blank sheet
    falloutRPG.clearCharacterSheet();
    
    if (typeof falloutRPG.saveCharacter === "function") {
        falloutRPG.saveCharacter(newName);
    } else {
        console.error("❌ ERROR: falloutRPG.saveCharacter is not defined.");
        return;
    }

    // ✅ Clear weapons for the new character
    if (typeof getWeaponStorageKey === "function") {
        localStorage.removeItem(getWeaponStorageKey());
    }
    
    if (typeof getAmmoStorageKey === "function") {
    localStorage.removeItem(getAmmoStorageKey()); // ✅ Clear ammo for new character
    console.log("🗑 Ammo cleared for new character.");
	} else {
	    console.warn("⚠ getAmmoStorageKey() is not available at this point.");
	}

    

    // ✅ Restore the dropdown and refresh it
    let characterSelect = document.createElement("select");
    characterSelect.id = "character-select";
    characterSelect.style.width = "100%";
    characterSelect.style.backgroundColor = "#fde4c9";
    characterSelect.style.borderRadius = "5px";
    characterSelect.style.border = "1px solid #ccc";
    characterSelect.style.padding = "5px";
    characterSelect.style.color = "black";
    ...
    inputField.replaceWith(characterSelect);

    // ✅ Attach event listener for switching characters
    setTimeout(() => {
        document.getElementById("character-select").addEventListener("change", function() {
            let selectedCharacter = this.value;
            if (selectedCharacter) {
                falloutRPG.switchCharacter(selectedCharacter);
            }
        });
    }, 50);
    ...
    setTimeout(() => {
        falloutRPG.updateCharacterDropdown();
        console.log("🔄 Character dropdown updated.");
    }, 100);

    // ✅ Switch to the new character after UI is updated
    setTimeout(() => {
        falloutRPG.switchCharacter(newName);
        console.log(`✅ Switched to new character: "${newName}"`);
    }, 150);

    // ✅ Reset Add Button
    addBtn.textContent = "➕";
    addBtn.removeEventListener("click", falloutRPG.saveNewCharacter);
    addBtn.addEventListener("click", falloutRPG.enterCharacterCreationMode);
};
...
createCharacterSheet(); 
	...
	// Ensure all inputs exist before attaching listeners 
		...
		setTimeout(() => { 
			...
			if (input.type === "checkbox") { 
				input.addEventListener("change", falloutRPG.saveCharacterData); 
				} 
			}); 
    ...
    document.getElementById(stat)?.addEventListener("input", () => { 
        falloutRPG.saveCharacterData();  // 🔄 Save everything
        updateDerivedStats();  
...
Object.keys(skillToSpecial).forEach(skill => { 
    ...
    ["Maximum HP", "Initiative", "Defense", "MeleeDamage"].forEach(id => attachManualOverride(id))
			...
			Object.keys(skillToSpecial).forEach(skill => { 
				...
				let skillTagInput = document.getElementById(`${skill}Tag`); 
				if (skillInput) { 
					skillInput.addEventListener("input", () => { falloutRPG.saveCharacterData();
 // ✅ Save skill changes 
		...
		if (skillTagInput) { 
			skillTagInput.addEventListener("change", () => { 
				falloutRPG.saveCharacterData(); // ✅ Save skill tag changes 
				renderWeaponTableUI(); 
			}); 
		} 
...
function renderCapsContainer() {
    ...
    function getCapsKey() {
    let currentCharacter = falloutRPG.getCurrentCharacter() || "default";
    return `fallout_rpg_${falloutRPG.sanitizeName(currentCharacter)}_Caps`;
}

const CAPS_KEY = getCapsKey();

    function loadCaps() {
    let key = getCapsKey();
    return localStorage.getItem(key) || '0';
}
let storedValue = loadCaps();
    ...
    document.removeEventListener("characterSwitched", updateCapsUI);
document.addEventListener("characterSwitched", updateCapsUI);


function updateCapsUI() {
    let key = getCapsKey();
    let capsValue = localStorage.getItem(key) || "0";

    if (CapsDisplay) CapsDisplay.textContent = capsValue;
    if (CapsInput) CapsInput.value = capsValue;
}
...
window.getWeaponStorageKey = function() {
	let currentCharacter = localStorage.getItem("fallout_rpg_selected_character") || "default";
    return `fallout_rpg_${falloutRPG.sanitizeName(currentCharacter)}_weapons`;
}
 

document.removeEventListener("characterSwitched", refreshWeaponTable);
document.addEventListener("characterSwitched", refreshWeaponTable);


function refreshWeaponTable() {
    let tbody = document.querySelector("#weapon-table-container tbody");
    if (!tbody) return;

    tbody.innerHTML = ""; // ✅ Clear previous weapons
...
```js-engine
document.removeEventListener("characterSwitched", refreshAmmoTable);
document.addEventListener("characterSwitched", refreshAmmoTable);


function refreshAmmoTable() {
    let container = document.getElementById("ammo-table-container");
    if (!container) {
        console.warn("⚠ Ammo table container not found. Refresh aborted.");
        return;
    }

    let tbody = container.querySelector("tbody");
    if (!tbody) {
        console.warn("⚠ Ammo table tbody not found. Refresh aborted.");
        return;
    }

    tbody.innerHTML = ""; // ✅ Clear previous ammo entries
...
function getAmmoStorageKey() {
    let currentCharacter = localStorage.getItem("fallout_rpg_selected_character") || "default";
    return `fallout_rpg_${falloutRPG.sanitizeName(currentCharacter)}_ammo`;
}
...
function saveAmmoTableState(ammo) { 
    let key = getAmmoStorageKey();
    localStorage.setItem(key, JSON.stringify(ammo)); 
}
...
async function renderAmmoTableUI() {
   const container = document.createElement('div');
	container.id = "ammo-table-container"; // ✅ Add ID for reference
    container.style.padding = '20px';
    container.style.border = '1px solid #e0c9a0';
    container.style.backgroundColor = '#325886';
    container.style.overflowX = 'auto';
...
```js-engine
function getArmorStorageKey(section) {
    let character = falloutRPG.getCurrentCharacter() || "default";
    return `fallout_rpg_${falloutRPG.sanitizeName(character)}_armor_${section}`;
}

function getPoisonDRKey() {
    let character = falloutRPG.getCurrentCharacter() || "default";
    return `fallout_rpg_${falloutRPG.sanitizeName(character)}_poison_dr`;
}
...
function saveArmorData(section, newData) {
    let storageKey = `${getArmorStorageKey(section)}_${section}`;
...
function renderArmorSection(section) {
    ...
    sectionContainer.id = `armor-section-${section.replace(/\s+/g, "-").toLowerCase()}`;
...
async function renderArmorSections() {
    ...
    const container = document.createElement('div');
    container.id = "armor-container"; // ✅ This allows easy replacement
    ...
    poisonDRContainer.id = "poison-dr-container";
    ...
    poisonDRInput.value = localStorage.getItem(getPoisonDRKey()) || '';

    // ✅ Save value when edited
    poisonDRInput.addEventListener('input', () => {
        localStorage.setItem(getPoisonDRKey(), poisonDRInput.value);
    });
...
document.removeEventListener("characterSwitched", refreshArmorSections);
document.addEventListener("characterSwitched", refreshArmorSections);


function refreshArmorSections() {
    let armorContainer = document.getElementById("armor-container");
    if (!armorContainer) return;

    armorContainer.innerHTML = ""; // Clear the existing armor UI
    setTimeout(async () => {
        let newArmorUI = await renderArmorSections();
        armorContainer.appendChild(newArmorUI);
    }, 100);
}
...
selectTd.style.border = '1px solid #ccc';
selectTd.style.padding = '8px';
selectTd.style.border = '1px solid #ccc';
selectTd.style.padding = '2px'; // Reduce padding
selectTd.style.width = '30px'; // Explicitly define a smaller width
selectTd.style.textAlign = 'center';
selectTd.style.alignItems = 'center';
...
row.appendChild(selectTd);
    ...
    ["name", "qty", "cost"].forEach((key, index) => {
        const td = document.createElement('td');
        td.style.border = '1px solid #ccc';
        td.style.padding = '8px';
